function [qrs_pos,sign,en_thres] = ppg_detect(ecg,varargin)
...
% try
    ...
    blocks = arrayfun(@(x) ecg(x-win:x)',maxloc,'UniformOutput',false);       % creates a maternal beat matrix
    [~,delay]=cellfun(@(x) min(x), blocks);
    maxloc = maxloc + (delay-win-1);
    maxloc = sort([maxloc extradet]);
