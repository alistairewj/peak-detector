function [qrs_pos,sign,en_thres] = jqrs_mod(ecg,varargin)
...
% %      % == Wavelet bandpass
% %      fsnew = 250;
% %      ppdata = resample(ecg,fsnew,fs); %resamples signal
% %      % preprocessing
% %      b = fir1(100,[10 70]./fsnew,'bandpass'); % 1 - 70 Hz bandpass
% %      ppdata = filtfilt(b,1,ppdata);    % zero-phase filtering
% %      ppdata = detrend(ppdata)';       % detrended data (otherwise filtering fails)
% %      
% %      % Scales (temporary hard-coded coefficients)
% %      s3 = [0	2.29E-005 0.0001373291 0.0004882813 0.0013160706 0.0029411316 ...
% %          0.0057144165 0.0099563599 0.0159072876 0.0236434937 0.0330276489 ...
% %          0.0436477661 0.0547866821 0.0654830933 0.0746841431 0.0814285278 ...
% %          0.0849952698 0.085018158 0.0814971924 0.074798584 0.0656318665 ...
% %          0.0549583435 0.0438308716 0.0332107544 0.0238189697 0.0160675049 ...
% %          0.010093689 0.005821228 0.0030212402 0.001373291 0.0005264282 ...
% %          0.0001602173 3.43E-005 3.81E-006];          
% %      featsig = conv(ppdata,s3,'same');   % actual wavelet filtering
% %      featsig = abs(featsig);
% %      featsig = filtfilt(ones(1,floor(fsnew*0.050)),1,featsig);
% %      yval = sort(featsig); yval = median(yval(end-100));
% %      featsig = featsig./yval;
% %      featsig = resample(featsig,fs,fsnew)'; % resample signal back to fs
% %      % plot
% %      plot(ecg.*10)
% %      hold on
% %      plot(featsig,'r')
